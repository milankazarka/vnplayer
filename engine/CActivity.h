//
//  CActivity.h
//  IAApp
//
//  Created by Milan Kazarka on 8/27/14.
//  Copyright (c) 2014 Milan Kazarka. All rights reserved.
//

/* Copyright (C) Milan Kazarka - All Rights Reserved
 * Unauthorized copying of this file, via any medium is strictly prohibited
 * Proprietary and confidential
 * Written by Milan Kazarka <milan.kazarka.office@gmail.com>, November 2014
 */

#ifndef __IAApp__CActivity__
#define __IAApp__CActivity__

#include "CObject.h"
#include "CCommon.h"

class CActivity : public CObject
{
public:
    TiXmlElement *meactivity;
    
    CActivity( TiXmlElement *eactivity );
};

#endif /* defined(__IAApp__CActivity__) */
